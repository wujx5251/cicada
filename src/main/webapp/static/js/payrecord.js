$(function () {
    ...
    $.getJSON("trade/getProducts", {}, function (rs) {
        var channel = $("#productType");
        if (rs) {
            $.each(rs, function (index, val) {
                channel.append('<option value="' + val.code + '">' + val.name + '</option>');
            });
    ...
    var table = $('.table').bootstrapTable({
        ...
        columns: [
            {field: 'recordId', title: '交易流水号'},
            {field: 'targetRecordId', title: '目标流水号'},
            ...
            {field: 'product', title: '支付产品', formatter: 'productFormater'},
            ...
            {field: 'payTime', title: '付款时间', formatter: 'timeFormater'},
            {
...
function detail(row) {
    return {
        ...
        onContentReady: function () {
            $.getJSON("payment/detail", {recordId: row.recordId}, function (rs) {
                if (rs) {
                    ...
                    $.each(params, function (i, v) {
                        ...
                        } else if ('product' == v.name) {
                            val = productFormater(val);
...
function productFormater(value, row, index) {
    if (value) {
        return $("#productType option[value='" + value + "']").text();
    }
}
